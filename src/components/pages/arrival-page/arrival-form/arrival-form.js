...
const ArrivalForm = ({addArrival, minors, workers}) => {
    ...
    if(minors !== undefined && workers !== undefined) {
      return (
        <form className="form" noValidate autoComplete="off">
            <FormLabel className="form__label">Прибытие</FormLabel>
            ...
            <FormControl required margin="normal">
              <InputLabel id="demo-simple-select-label">Медицинский работник</InputLabel>
              <Select  className = "form__select" labelId="demo-simple-select-label" id="demo-simple-select"
              value={medic}
            ...
            <FormControl required margin="normal">
              <InputLabel id="demo-simple-select-label">Социальный работник</InputLabel>
              <Select  className = "form__select" labelId="demo-simple-select-label" id="demo-simple-select"
              value={social}
              onChange={ e => setSocial(e.target.value)}>
                {workers.map((worker) => {
                  return (
                  <MenuItem key={worker.id_worker + 20000} value={worker.id_worker}>{worker.Surname} {worker.Name}</MenuItem>
                  )
                })}
              </Select>
            </FormControl>
          ...
          onClick={() => SendInfo()}>Добавить прибытие</Button>
        </form>
      )
    } else{
      return (
